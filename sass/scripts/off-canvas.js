 ...
 * Copyright (c) 2014 David Heidrich
...
(function () {
     ...
     */
    HFLayout.init = function (document, options) {

        $.extend(HFLayout.SETTINGS, options);

        // Prevent accidental touches
        document.body.addEventListener('touchmove', function () {
            HFLayout.TOUCHMOVE = true;
        });
        document.body.addEventListener('touchstart', function () {
            HFLayout.TOUCHMOVE = false;
        });
        // Measure scrollbar width
        HFLayout.CONST_SCROLLBAR_WIDTH = HFLayout.getScrollBarWidth();

    };

    // Some constants:
    HFLayout.CONST_SCROLLBAR_WIDTH = 15;
    HFLayout.CONST_ANIM_DARKENER = 200;
    HFLayout.CONST_CANVAS_TOGGLE = 'toggled-canvas';
     ...
     * @returns {boolean}
     */
    HFLayout.isPartOfNode = function (target, parent) {
        if (!target || !parent) {
            return false;
        }
        var now = target;
     ...
     * @returns {*}
     */
    HFLayout.parentsUntil = function (target, func) {
        if (!target) {
            return false;
        }
        var now = target;
     ...
     * @returns {number}
     */
    HFLayout.getScrollBarWidth = function () {
...
(function (HFLayout) {
     ...
     * @constructor
     */
    HFLayout.Scrollable = function (ScrollId, OverflowContainer, offsetStart) {
     ...
     * @param ContainerId
     * @returns {HFLayout.Toggleable}
     * @constructor
     */
    HFLayout.Toggleable = function (ContainerId) {
        ...
        var listener = function (e) {
            ...
            if (ref) {
                ...
                if (widget instanceof HFLayout.Widget && widget.getAsync()) {
                    future = target.hfWidgetInstance.runAsync().then(function (r) {
                        if (r instanceof HTMLElement) {
                            var id = HFLayout.guid();
                            r.id = id;
            ...
            if (parent) {
                maybeToggleNode = HFLayout.parentsUntil(target, function (n) {
        ...
        HFLayout.SETTINGS.clickEvents.forEach(function (e) {
     ...
     * @constructor
     */
    HFLayout.CreateOffCanvas = function (NavigationId, ToggleNavigationId, Darkener, factor) {
        ...
        var doc = document, touched = 0, body = doc.getElementById('SiteBody'), navigationContainer = doc.getElementById(NavigationId),
            ...
            OPEN_CLASS = 'open', INIT_CLASS = 'init', TOGGLE_CLASS = HFLayout.CONST_CANVAS_TOGGLE,
            ANIM_DELAY = HFLayout.CONST_ANIM_DARKENER, shouldNotTouch = function () {
                return window.innerWidth >= HFLayout.SETTINGS.smallBreakpoint;
            };
        ...
        var closer = function (e) {
            if (navigationContainer.classList.contains(OPEN_CLASS)) {
                if (!HFLayout.isPartOfNode(e.target, toggler) && !HFLayout.isPartOfNode(e.target, navigationContainer)) {
                    togglerF(e);
                }
            }
        };
...
(function (HFLayout, window, $) {
     ...
     * @constructor
     */
    HFLayout.SetupPositionNearby = function (target, elementToPosition, collisionContainer) {
     ...
     * @returns {HFLayout.CreateTooltip}
     * @constructor
     */
    HFLayout.CreateTooltip = function (DelegateContainer) {
     ...
     * @param WidgetId
     * @returns {HFLayout.Widget}
     * @constructor
     */
    HFLayout.Widget = function (WidgetId) {
        var self = this;
        self.async = null;
        ...
        /**
         * @returns {null|*|HFLayout.Widget.async}
         */
    ...
    HFLayout.CreateWidget = function (id) {
        return new HFLayout.Widget(id);
    };
     ...
     * @constructor
     */
    HFLayout.CreateDropdown = function (DelegateContainer, Darkener) {
        var doc = document, container = doc.getElementById(DelegateContainer),
            ...
            currentOpen = null, darkener = document.getElementById(Darkener), close = function () {
                if (window.getComputedStyle(currentOpen).position === 'fixed') {
                    setTimeout(function () {
                        container.classList.remove(HFLayout.CONST_CANVAS_TOGGLE);
                    }, HFLayout.CONST_ANIM_DARKENER);
                }
        ...
        var func = function (e) {
            ...
            if (currentOpen && !HFLayout.isPartOfNode(e.target, currentOpen)) {
                close();
                return func(e);
            }
            ...
            if (targetHas || parentHas) {
                ...
                if (!dropdownContainerElement && widget instanceof HFLayout.Widget && widget.getAsync()) {
                ...
                future.then(function (dropdownContent) {
                    ...
                    if (isAbsolute) {
                        // Check collision:
                        HFLayout.SetupPositionNearby(target, dropdownContent, document.body);
                    } else {
                        container.classList.add(HFLayout.CONST_CANVAS_TOGGLE);
                ...
                if (currentOpen) {
                    ...
                    if (!HFLayout.isPartOfNode(e.target, currentOpen)) {
                        return close();
                    }
                }
            }
    ...
    HFLayout._modalInstances = [];

    // Global Widget keydown listener
    window.addEventListener('keydown', function (e) {
        if (27 === e.keyCode) {
            var lastModal = HFLayout._modalInstances[HFLayout._modalInstances.length - 1];
            if (lastModal) {
    ...
    HFLayout.MODAL_JUST_OPENED = false;

    /**
     * A Modal
     * @param DelegateContainer
     * @param options
     * @returns {HFLayout.Modal}
     * @constructor
     */
    HFLayout.Modal = function (DelegateContainer, options) {
        ...
        function _removeModalFromStack(n) {
            var t = HFLayout._modalInstances.indexOf(n);
            if (t > -1) {
                HFLayout._modalInstances.splice(t, 1);
                if (0 === HFLayout._modalInstances.length) {
        ...
        var switchModals = function (co, last) {
            co.prevModal = last;
            HFLayout._modalInstances.push(co);
        ...
        var createWidget = function (e) {
            ...
            var targetContent, future, widget, target, hasTarget = true,
                isHtmlElement = e instanceof HTMLElement, isWidget = e instanceof HFLayout.Widget;
            if (isHtmlElement || isWidget) {
                ...
                if (target.hasAttribute(ATTR_CREATE_NEW) && !e.newInstance) {
                    var newInstance = new HFLayout.Modal(DelegateContainer)
            ...
            HFLayout.MODAL_JUST_OPENED = true;
            ...
            if (!modalContainer) {
                ...
                var closeModalFunction = function (e) {
                    if (loading || HFLayout.MODAL_JUST_OPENED) {
                        return;
                    }
                    if (HFLayout.TOUCHMOVE) {
                        return;
            ...
            if (0 === HFLayout._modalInstances.length) {
                ...
                container.style.paddingRight = HFLayout.CONST_SCROLLBAR_WIDTH + 'px';
            }
